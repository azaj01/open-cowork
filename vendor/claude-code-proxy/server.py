...
async def create_message(
    ...
    try:
        ...
        # Determine upstream provider to use.
        # Some upstream model IDs (e.g. gpt-5.3-codex) may arrive without "openai/" prefix.
        model_name = request.model or ""
        litellm_model_name = str(litellm_request.get("model", "") or "")
        is_openai_model = model_name.startswith("openai/") or litellm_model_name.startswith("openai/")
        is_gemini_model = model_name.startswith("gemini/") or litellm_model_name.startswith("gemini/")
        is_anthropic_model = model_name.startswith("anthropic/") or litellm_model_name.startswith("anthropic/")
        should_use_openai_provider = is_openai_model or (
            PREFERRED_PROVIDER == "openai" and bool(OPENAI_API_KEY)
        )

        if should_use_openai_provider:
            litellm_request["api_key"] = OPENAI_API_KEY
            # Normalize model prefix for OpenAI-specific handling below.
            ...
            if OPENAI_BASE_URL:
                ...
                logger.debug(
                    f"Using OpenAI API key and custom base URL {OPENAI_BASE_URL} for model: {request.model} (store=False)"
                )
            else:
                logger.debug(f"Using OpenAI API key for model: {request.model} (store=False)")
        ...
        if isinstance(litellm_request.get("model"), str) and litellm_request["model"].startswith("openai/") and "messages" in litellm_request:
